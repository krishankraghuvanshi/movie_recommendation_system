...
if not TMDB_API_KEY:
    print("WARNING: TMDB_API_KEY missing. Put it in .env or Render Environment Variables as TMDB_API_KEY=xxxx")
...
DF_PATH = resolve_data_path("df.pkl")
INDICES_PATH = resolve_data_path("indices.pkl")
TFIDF_MATRIX_PATH = resolve_data_path("tfidf_matrix.pkl")
TFIDF_PATH = resolve_data_path("tfidf.pkl")
...
async def tmdb_movie_details(movie_id: int) -> TMDBMovieDetails:
    data = await tmdb_get(f"/movie/{movie_id}", {"language": "en-US", "append_to_response": "videos,credits"})
    ...
    # Extract Cast
    cast_list = []
    credits = data.get("credits", {}).get("cast", [])
    for c in credits[:8]:  # Top 8 actors
        cast_list.append({
            "id": c.get("id"),
            "name": c.get("name"),
...
# ---------- ACTOR MOVIES ----------
@app.get("/actor/{actor_id}/movies", response_model=List[TMDBMovieCard])
async def actor_movies(actor_id: int, limit: int = Query(24, ge=1, le=50)):
    """
    Fetch popular movies featuring the specified actor.
    """
    discover = await tmdb_get(
        "/discover/movie",
        {
...
async def search_bundle(
    ...
    """
    resolved_tmdb_id: Optional[int] = tmdb_id
    if resolved_tmdb_id is None:
        ...
        resolved_tmdb_id = int(best["id"])

    details = await tmdb_movie_details(int(resolved_tmdb_id))
